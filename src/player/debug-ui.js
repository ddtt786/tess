...
const state = observable({
  ...
  heights: {
    'run-control': DEFAULT_SECTION_HEIGHT, 'data-vars': DEFAULT_SECTION_HEIGHT,
    'data-signals': DEFAULT_SECTION_HEIGHT, 'objects-tree': DEFAULT_SECTION_HEIGHT,
...
const control = (action) => {
  try { action(engine()); } catch (error) { failed('실행 제어', error); }
...
const setEntityNumber = (name) => (objectId, text) => {
  ...
  if (!live || !Number.isFinite(value)) { touch(); return; }
  try {
...
function blockNode(block) {
  return h('li', { key: block.id, 'data-block-id': block.id, class: blockClass(block) }, [
    h('span', { class: 'block-type' }, blockLabel(block)),
    ...(block.params || [])
      .filter((param) => param && typeof param === 'object' && param.type)
      .map((param, i) => h('ul', { key: 'p' + i, class: 'block-param' }, blockNode(param))),
    ...(block.statements || [])
      .filter((branch) => Array.isArray(branch) && branch.length > 0)
      .map((branch, i) => h('ul', { key: 's' + i, class: 'block-body' }, branch.map(blockNode))),
  ]);
}
...
const emptyRow = (text) => h('li', { key: 'empty', class: 'debug-empty' }, text);

// ============================================================================
//  4. 탭
// ============================================================================
function RunTab() {
  const field = (id, label, value, options) => h('div', { class: 'debug-field' }, [
    h('label', { for: id }, label),
    h('select', {
      id,
      value,
      onChange: (event) => { state.env[id.replace('env-', '')] = event.target.value; },
    }, options.map(([v, text]) => h('option', { key: v, value: v, selected: v === value }, text))),
  ]);
  ...
  return sections([
    {
      ...
      title: '실행 제어',
      body: [
        h('p', { class: 'debug-run-state' + (state.runState ? ' state-' + state.runState : '') }, [
          h('span', { class: 'dot' }),
          STATE_TEXT[state.runState] || '실행기를 기다리는 중…',
        ]),
        h('div', { class: 'debug-run-buttons' }, [
          h('button', {
            type: 'button', id: 'run-btn', onClick: doRun,
            disabled: !state.runState || state.runState === 'run',
          }, '시작하기'),
          h('button', {
            type: 'button', id: 'pause-btn', onClick: doPause,
            disabled: !state.runState || state.runState === 'stop',
          }, state.runState === 'pause' ? '이어서 하기' : '일시정지'),
          h('button', {
            type: 'button', id: 'stop-btn', onClick: doStop,
            disabled: !state.runState || state.runState === 'stop',
          }, '정지하기'),
        ]),
        h('p', { class: 'debug-note' }, [
          '정지한 뒤에도 ', h('b', null, '시작하기'), ' 로 처음부터 다시 실행할 수 있습니다.',
        ]),
        h('p', {
          class: 'debug-note debug-pick-hint' + (state.picking ? ' active' : ''),
          id: 'pick-hint',
        }, state.picking
    ...
    },
    {
      ...
      body: [
        field('env-boost', '부스트 모드', state.env.boost, [
          ['', '실제 값 그대로'], ['true', '켜짐 (참)'], ['false', '꺼짐 (거짓)'],
        ]),
        h('p', { class: 'debug-note' }, [
        ...
        field('env-device', '기기 종류', state.env.device, [
          ['', '실제 값 그대로'], ['desktop', '컴퓨터'], ['tablet', '태블릿'], ['mobile', '스마트폰'],
        ]),
        field('env-touch', '터치 지원', state.env.touch, [
          ['', '실제 값 그대로'], ['true', '지원함 (참)'], ['false', '지원 안 함 (거짓)'],
        ]),
      ],
    },
  ]);
...
function listRow(entry) {
  ...
  return h('li', { key: entry.id, class: 'debug-list-head' }, [
    h('button', {
      type: 'button', class: expandClass(key), onClick: () => toggleExpanded(key),
    }, entry.name), ' ',
    h('span', { class: 'tag' }, entry.scope + ' · ' + entry.kind), ' ',
...
function listItemsRow(entry) {
  ...
  return h('li', { key: entry.id + ':items', class: 'debug-items-row' },
    h('div', { class: 'debug-list-items' }, [
      h('button', {
        type: 'button', class: 'debug-mini-btn debug-add-btn',
        onClick: () => addListItem(entry.id),
      }, '항목 추가'),
      array.length === 0
        ? h('p', { class: 'debug-empty' }, '비어 있습니다.')
        : h('ol', { class: 'debug-list-ol' }, array.map((item, index) => h('li', { key: index }, [
          h('span', { class: 'debug-list-index' }, index + 1),
          editable(key + ':' + index, (array[index] || {}).data,
            (text) => setListItem(entry.id, index, text)),
          h('button', {
            type: 'button', class: 'debug-mini-btn', title: '이 항목 지우기',
...
function functionCodeRow(fn) {
  ...
  return h('li', { key: fn.id + ':code', class: 'debug-items-row' },
    h('div', { class: 'debug-list-items debug-func-code' },
...
function DataTab() {
  const varRows = [];
  if (state.variables.length === 0) varRows.push(emptyRow('변수나 리스트가 없습니다.'));
  else {
    for (const entry of state.variables) {
      if (entry.source.variableType !== 'list') { varRows.push(variableRow(entry)); continue; }
      varRows.push(listRow(entry));
      if (state.expanded === 'list:' + entry.id) varRows.push(listItemsRow(entry));
    }
  }
  ...
  return sections([
    {
      id: 'data-vars',
      title: '변수 · 리스트',
      body: h('div', { id: 'var-list' }, h('ul', { class: 'debug-rows' }, varRows)),
    },
    {
      id: 'data-signals',
      title: '신호',
      body: h('div', { id: 'signal-list' }, state.messages.length === 0
        ? empty('신호가 없습니다.')
        : h('ul', { class: 'debug-rows' }, state.messages.map((message) => h('li', { key: message.id }, [
          h('span', { class: 'key' }, message.name),
          h('button', {
            type: 'button', class: 'debug-send-btn', onClick: () => sendSignal(message.id),
          }, '보내기'),
        ])))),
    },
    {
      id: 'data-functions',
      title: '함수',
      body: h('div', { id: 'function-list' }, h('ul', { class: 'debug-rows' }, fnRows)),
    },
  ]);
}
...
function objectInfoRows() {
  ...
  const infoRow = (label, body) => h('li', { key: label }, [
    h('span', { class: 'key' }, label), ' ', body,
  ]);

  if (!live) {
    return [infoRow('', h('span', { class: 'debug-empty' }, state.currentId
  ...
  for (const field of ENTITY_FIELDS) {
    ...
    rows.push(infoRow(field.label, field.set
      ? editable('entity:' + field.key, value, (text) => field.set(state.currentId, text))
  ...
  if (!isText) {
    ...
    rows.push(infoRow('모양', chooser(pictures,
      live.entity.picture ? live.entity.picture.id : '',
      (id) => setPicture(state.currentId, id))));
    rows.push(infoRow('모양 번호', h('span', { class: 'val' }, pictureInfo(live).index)));
  }

  rows.push(infoRow('보이기', toggle(call(live.entity, 'getVisible', true),
    (next) => setEntityVisible(state.currentId, next), ['보임', '숨김'])));
  ...
  if (!isText) {
    ...
    rows.push(infoRow('글 내용', editable('entity:text', live.entity.text,
      (text) => setEntityText(state.currentId, text))));
  }
  return rows;
}
  ...
  return sections([
    {
      id: 'objects-tree',
      title: '장면 · 오브젝트',
      body: h('div', { id: 'scene-tree' }, state.scenes.length === 0
        ? empty('불러오는 중…')
        : state.scenes.map((scene) => h('div', { key: scene.id }, [
          ...
          h('ul', { class: 'debug-object-list' }, scene.objects.length === 0
            ? h('li', { class: 'debug-empty' }, '(오브젝트 없음)')
            : scene.objects.map((object) => h('li', { key: object.id }, h('button', {
              type: 'button',
              class: 'debug-object-btn' + (state.currentId === object.id ? ' active' : ''),
    ...
    },
    {
      ...
      body: h('div', { id: 'object-info' }, h('ul', { class: 'debug-rows' }, objectInfoRows())),
    },
    {
      id: 'objects-blocks',
...
function blockTree() {
  ...
  return currentThreads.map((thread, index) => h('div', { key: index, class: 'debug-thread' }, [
  ...
  return sections([
    {
      id: 'errors-log',
      title: '오류 로그',
      body: h('div', { id: 'error-log' }, state.errors.length === 0
        ? empty('아직 오류가 없습니다. entryjs 가 실행 중 panic 을 내면 여기와 이 서버를 띄운 터미널에 같이 찍힙니다.')
        : state.errors.map((error, index) => h('details', {
          key: error.key, class: 'error-item', open: index < 3,
        }, [
          h('summary', null, '[' + error.at + '] ' + error.kind + ': ' + error.message),
...
function Panel() {
  return [
    ...
    h('div', { class: 'debug-header' }, [
      h('h2', null, '디버그'),
      h('button', {
        id: 'debug-close', type: 'button', 'aria-label': '닫기', onClick: closePanel,
      }, '×'),
    ]),
    h('div', { class: 'debug-tabs', role: 'tablist' }, TABS.map((tab) => h('button', {
      key: tab.id,
      type: 'button',
      role: 'tab',
      class: 'debug-tab',
      ...
      tab.id === 'errors'
        ? h('span', {
          id: 'error-count', class: 'badge', hidden: state.errors.length === 0,
        }, state.errors.length)
        : null,
    ]))),
  ...
  for (const object of objects) {
    ...
    try {
      ...
      if (Math.abs(point.x - entity.getX()) <= halfWidth
        && Math.abs(point.y - entity.getY()) <= halfHeight) return object;
    } catch (error) { /* 이 오브젝트는 건너뛴다 */ }
  }
  return null;
}
