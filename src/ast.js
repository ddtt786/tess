...
semantics.addOperation("ast", {
  ...
  // --- 오브젝트 속성 --------------------------------------------------------
  PropertyDecl_defaultCostumeSized(
    _default,
    _costume,
    id,
    file,
  ...
  PropertyDecl_rotation(_rotation, method) {
    return {
      type: "Property",
      name: "rotation",
      value: { type: "Keyword", name: method.sourceString },
      loc: at(this),
    };
  },
  ...
  PropertyDecl_boxSize(_size, width, height) {
    return {
      type: "BoxSize",
      width: width.ast().value,
      height: height.ast().value,
      loc: at(this),
    };
  },
  ...
  LooksStatement_orderFirst(_order, _first) {
    return { type: "Order", to: "first", loc: at(this) };
  },
  LooksStatement_orderLast(_order, _last) {
  ...
  CompareExpr_eq(l, _op, r) {
    return binary(this, "==", l, r);
  },
  CompareExpr_ne(l, _op, r) {
    return binary(this, "!=", l, r);
  },
  CompareExpr_le(l, _op, r) {
    return binary(this, "<=", l, r);
  },
  CompareExpr_ge(l, _op, r) {
    return binary(this, ">=", l, r);
  },
  CompareExpr_lt(l, _op, r) {
    return binary(this, "<", l, r);
  },
  CompareExpr_gt(l, _op, r) {
    return binary(this, ">", l, r);
  },
  ...
  stringLiteral(_open, chars, _close) {
    return {
      type: "String",
      value: chars.children.map((c) => c.ast()).join(""),
      loc: at(this),
    };
  },
  stringChar_unicodeEscape(_prefix, h1, h2, h3, h4) {
    const hex =
      h1.sourceString + h2.sourceString + h3.sourceString + h4.sourceString;
  ...
  colorLiteral(_hash, _body) {
    return {
      type: "Color",
      value: this.sourceString.toLowerCase(),
      loc: at(this),
    };
  },
