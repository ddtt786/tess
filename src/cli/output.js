...
const wrap = (open, close) => (text) => (COLORED ? `\u001b[${open}m${text}\u001b[${close}m` : String(text));
...
/** 명령 하나를 시작한다 */
export function begin(command, file) {
...
const stepLine = (symbol, text) => `${grey(S_BAR)}  ${symbol}  ${text}`;
const timed = (label, ms) => `${padTo(label, STEP_LABEL_WIDTH)} ${dim(duration(ms).padStart(6))}`;

/**
 * 오래 걸리는 일 하나. 터미널이면 제자리에서 돌다가, 끝나면 다른 단계와 똑같은
 * 모양의 한 줄로 바뀐다. 파이프로 넘길 때는 돌지 않고 끝난 줄만 남는다.
 */
export function working(title) {
  ...
  return {
    done: (label) => finish(green(S_STEP_SUBMIT), timed(label, performance.now() - started)),
    fail: (message) => finish(red(S_ERROR), message),
  };
}
 ...
 */
function diagnostic(label, item) {
...
/** 에러·경고 묶음을 찍는다 */
export function report(label, diagnostics, kind) {
