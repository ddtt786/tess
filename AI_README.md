...
| **오브젝트** | 장면·오브젝트 목록, 고른 오브젝트의 **정보**(좌표·크기·방향·모양·보이기…)와 컴파일된 블록 트리                                                                                                                                                    |
...
접습니다**. 항목이 100개여도 펼친 칸은 정해진 높이 안에서 스크롤되고, 항목마다 고치기·지우기,
맨 아래에 넣기가 있습니다.
...
**arrow-js 에서 목록 줄 안의 값은 반드시 함수로 넣어야 합니다.** `.key()` 로 키를 준 줄은
다시 그릴 때 DOM 을 그대로 다시 쓰고, 그 안에서는 **함수로 넣은 것만** 다시 평가합니다.
그래서 `${liveValue(entry)}` 처럼 값을 미리 꺼내 두면 그 줄만 첫 값에 얼어붙습니다 —
`${() => liveValue(entry)}` 로 넣어야 합니다. 변수 값·리스트 항목·오브젝트 정보가 전부
이 규칙을 따릅니다(`editable` 이 값 대신 게터를 받는 이유).
