...
export class BlockDragStrategy implements IDragStrategy {
  ...
  /** Whether the dragged stack is drawn by a stand-in (see drag_proxy.ts). */
  ...
  startDrag(e?: PointerEvent | KeyboardEvent) {
    ...
    // A long stack dragged by pointer is drawn by a stand-in and left unmarked.
    this.proxied =
      e instanceof PointerEvent && this.block.canUseDragProxy();
    this.block.setDragging(true, !this.proxied);
  ...
  endDrag(
    ...
    if (
      ...
      this.block.setDragging(false, !this.proxied);
