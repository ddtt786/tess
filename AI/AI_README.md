...
| 함수 밖의 `return`                                                   | 14   | 에러 |
| 반복문 밖의 `break` / `continue` / `skip`                            | 5.3  | 에러 |
...
#### 무대에서 오브젝트 고르기 — 크롬 개발자 도구식 고르기 단추

패널 머리의 고르기 단추를 켜면 **마우스를 댄 오브젝트가 오브젝트 탭에 그대로 열린다.**
누르면 거기에 붙박이고, 한 번 더 누르면 풀려서 다시 따라간다(Esc 로도 끈다). 크롬
개발자 도구의 element 고르기와 같은 성격이다. 예전에는 Ctrl+Shift+클릭 한 번에 하나씩
집는 방식이었는데, 어느 오브젝트인지 확인하려면 매번 다시 눌러야 했다.

도구가 꺼져 있으면 무대 이벤트에 아예 손대지 않는다. 켜져 있는 동안에는
`pointerdown`·`mousedown` 뿐 아니라 뒤따르는 `mouseup`·`click` 까지 삼킨다 — 살펴보려고
누른 것이지 작품을 진행시키려고 누른 게 아니다.

같은 오브젝트 위에서 움직일 때는 패널을 다시 그리지 않고(`shownId` 비교), 아무것도 없는
자리로 가면 마지막에 고른 것을 그대로 열어 둔다 — 패널로 마우스를 가져가는 동안 골라 둔
것이 사라지면 살펴볼 수가 없다.

고르기 자체가 잘 안 먹던 이유는 브라우저에서 직접 재 보니 세 가지였다.
...
지금은 이벤트마다 `state.inspecting` 하나만 보고 새로 판단한다(`ours`). 도구가 켜져
있는 동안 무대에서 일어난 일은 언제나 우리 것이므로 따로 창을 열 필요가 없고,
`preventDefault` + `stopPropagation` 으로 실행기까지 못 가게 막으므로 함수를 바꿔치기할
일도 없다.

완전히 투명한 오브젝트(`entity.effect.alpha === 0`)는 건너뛴다. 무대를 덮는 투명한
판이 하나 있으면 어디를 눌러도 그것만 잡혀서 역시 "고르기가 안 되는 것" 처럼 보인다.

**투명도 효과만으로는 모자란다.** 모양 자체가 비어 있는 960x540 짜리 판은
`effect.alpha` 가 1 이라 이 검사를 그냥 통과하고, 상자로만 재는 고르기는 무대 전체를
그 판에게 내준다 — 그 밑의 오브젝트는 영영 고를 수 없다. 그래서 실행기에게
`hitTest(entity, x, y)` 로 **모양의 그 자리에 정말 그림이 있는지** 묻고, 답이 오면 그걸
그대로 쓴다. tessvm 은 `when_object_click` 이 쓰는 그 판정(`collision.touchingMouse`)을
그대로 내주고, entryjs 는 임의의 점에 대한 픽셀 판정을 내주지 않으므로 `null` 을 돌려
상자 판정으로 떨어진다.
...
**패널 머리의 고르기 단추**를 켜면 마우스를 댄 자리의 오브젝트가 오브젝트 탭에서
열립니다. 누르면 붙박고, 한 번 더 누르면 풀립니다. 고르는 동안에는 작품의 "오브젝트를
클릭했을 때" 가 돌지 않도록 이벤트를 무대까지 내려보내지 않습니다 — 디버깅하려고 누른
것이지 작품을 진행시키려고 누른 게 아니기 때문입니다.
...
**떼어 놓은 블록 뭉치는 아무것도 알리지 않습니다.** 실행된 적 없는 코드를 주석으로
옮긴 것은 알려 줄 일이 아니고, 그걸 옮기다 나온 진단도 마찬가지입니다 — 워크스페이스에
값 블록 하나가 떨어져 있는 것만으로 "문장 블록 `calc_rand` 는 아직 옮길 수 없습니다" 가
뜨던 자리입니다. 그 뭉치를 옮기는 동안 늘어난 경고·주의는 되돌려 놓습니다. 연결된
코드에서 나온 것은 그대로 셉니다.

**`comment` 는 블록이 아닙니다.** 블록에 붙인 메모는 그 블록의 `comment` 로 들어가고,
아무 데도 안 붙인 메모만 `type: "comment"` 인 스레드 하나로 저장됩니다. 이건 못 옮기는
블록이 아니라 그냥 메모라서, 내용을 `#` 주석으로 그대로 옮깁니다(빈 메모는 버립니다).

**블록에 붙은 메모도 되돌립니다.** 컴파일러는 같은 줄 뒤에 적은 주석과 문장 바로 위에
붙인 주석 묶음을 모두 그 문장의 **첫 블록**에 답니다(`comments.ts` 의 `attachInFile`).
되돌릴 때는 그 자리에서 문장 **위**의 `#` 줄로 꺼냅니다 — 같은 줄에 적었던 것도 위로
올라오지만 내용은 그대로고, 다시 컴파일하면 같은 블록으로 돌아갑니다. 이벤트의 메모는
hat 블록이, 함수 정의의 메모는 `function_create` 가 들고 있어서 각각 `when …` 줄과
`function …` 줄 위로 나옵니다.
...
**복사된 함수는 같은 매개변수·지역변수 id 를 그대로 씁니다.** 합작 작품을 만드는
합치기 도구(entryMergy)는 함수를 통째로 복사하면서 `stringParam_xxxx` 블록 타입과
`함수id_해시` 지역변수 id 를 **바꾸지 않습니다.** `redred.ent` 는 매개변수 블록 타입
837개 중 343개, 지역변수 id 230개 중 141개가 여러 함수(많게는 12개)에 겹쳐 있습니다.

엔트리는 이래도 잘 돕니다 — 매개변수는 **돌고 있는** 함수의 `register.paramMap` 에서,
지역변수는 그 실행기의 `executor.localVariables`(그 함수 목록의 복사본)에서 찾기
때문입니다. 복사본마다 자기 표가 있으니 같은 키가 열두 번 나와도 서로를 건드리지
않습니다.

되돌리기가 이 둘을 **작품 하나에 표 하나**로 들고 있으면 같은 키를 마지막에 선언한
함수만 이깁니다. 나머지 복사본은 매개변수가 "다른 함수의 것" 으로 판정되어 빈 값이
되고, 지역변수는 남의 이름을 가리켜 컴파일 에러가 납니다. `redred.ent` 에서 함수
400개 중 186개가 그렇게 망가졌고(참조 9126군데), 이징·카메라 함수가 인자를 0 으로
읽어 화면이 튀었습니다. 두 표 모두 **함수 id 로 한 겹 감싸서**(`Map<함수id, Map<키,
이름>>`) 엔트리와 같은 자리에서 찾습니다.

지금 쓰고 있는 함수가 선언하지 않은 키는 여전히 "다른 함수의 것" 입니다 — 매개변수는
빈 값(엔트리도 `params[undefined]`), 지역변수는 읽기가 `0`(`Entry.Func.getValue` 의
`|| 0`), 쓰기는 문장째 빠집니다(엔트리는 거기서 오류를 던져 값이 들어가지 않습니다).
...
**값 자리에 놓인 흐름 블록**도 그런 트릭입니다. `~이(가) 될 때까지 기다리기` 의 판단
칸에 `(이)가 아니다` 로 감싼 `이번 반복 건너뛰기`·`반복 중단하기` 를 꽂는 형태인데,
엔트리는 블록을 돌리기 전에 값 칸부터 읽으므로 그 자리에서 `continueLoop`·`breakLoop`
가 불립니다. 둘 다 실행 스코프를 **감싸는 반복 블록까지 되감아 놓고**, `(이)가 아니다`
가 그 반환값을 거짓으로 뒤집어 기다리기가 제 스코프를 돌려주므로, 실행기의 어느 분기에도
안 걸려 그 프레임에 그대로 다음 바퀴로 갑니다 — 그래서 둘 다 `skip` 으로 옮깁니다.

반복 밖에 있으면 되감을 스코프가 없어 스코프가 그 자리에 그대로 남고, 기다리기가 돌려준
...
`@저장`·`@비동기저장` 함수는 선언으로 남기지 않고 그 호출을 `save`·`save async` 로,
`@확장프로그램 == 1` 비교는 `can_save` 로 되돌립니다(AI_SPEC-ADDENDUM.md 1.7).
...
`isExactNumber`(`String(Number(x)) === x` 이면서 `numberLiteral` 의 모양일 것)가 그
판단을 합니다. 리스트·글자 순번도 `text` 블록에 담겨 있을 수 있어서, 그것까지 알아봐야
`("3" - 1)` 같은 게 안 남습니다.

**지수 표기는 숫자가 아닙니다.** Tess 의 `numberLiteral` 은 `digit+ ("." digit+)?` 뿐이라
`1e-10` 을 읽지 못하는데, 자바스크립트 `Number` 로는 그대로 돌아오는 값이라 앞의 검사만
통과하고 숫자로 적혀 나갔습니다. `hiphop.ent` 의 지수 상수 하나가 그렇게 되돌린 소스를
컴파일되지 않는 상태로 만들었습니다 — 같은 식의 이웃한 값들은 `"2.08767569878681e-09"`
처럼 지수 자리에 `0` 이 붙어 있어서 글자 검사에 걸려 문자열로 남았고, `0` 이 없는 하나만
빠져나갔습니다. 이제 글자 모양까지 보므로 이런 값은 문자열로 남습니다 — 원래 블록도
`text` 였으니 다시 컴파일하면 같은 블록으로 돌아갑니다.

자바스크립트 숫자를 직접 적는 자리(`tessNumber` — 오브젝트 좌표·크기·소리 길이)는
`String(v)` 가 지수 표기를 뱉을 수 있으므로(`1e-7`), 소수점 표기로 펴서 적습니다.
...
| 지워진 오브젝트를 가리키는 블록       | `_missing_object_<id>` 로 적고 경고합니다 (`touching`·`go` …) |
...
`insert`·`delete`·`home`·`end`·`pageup`·`pagedown`·`capslock`·`f1`~`f12`·`numpad0`~`numpad9`
에 이름을 주고, **그래도 이름이 없는 코드는 번호 그대로** 적습니다(`when key "145"`).
`keyCodeOf` 가 숫자 문자열을 그대로 받으므로 그 작품도 왕복합니다.
...
**지워진 오브젝트를 가리키는 블록 — `_missing_object_<id>`.** `locate`·`reach_something`
처럼 대상 오브젝트를 고르는 칸은 오브젝트 id 를 들고 있습니다. 그 오브젝트가 이미
없으면 되돌리기가 `go "_missing_object_7o00"` 로 적고, 컴파일러가 그 이름을 알아보고
**아이디만 되돌려 놓은 뒤 주의로** 알립니다. 변수·리스트의 `_missing_var_*` 와 같은
규칙입니다.

이름을 그냥 남기지 않고 표를 붙이는 이유는 **손으로 적은 오타와 갈라 놓기 위해서**
입니다. 엔트리는 오브젝트 칸을 아이디로만 찾으므로(`Entry.container.getObject` 는
이름을 보지 않습니다), 없는 이름은 실행해도 영영 찾을 수 없는 자리입니다 — 그래서
`go "없는이름"` 은 여전히 에러입니다. 반면 표가 붙은 자리는 엔트리가 실제로 들고
있는 상태이므로 그대로 통과시킵니다.

이걸 에러로 버리면 조건 칸이 빠지면서 감싸고 있던 `if`/`if_else` 까지 함께 사라져
`else` 쪽 동작도 잃었습니다 — `dog.ent` 의 `카드배치` 는 몸통이 통째로 비었고,
`파워업` 장면의 카드 생성 뭉치도 이 때문에 빠졌습니다. 이 처리로 `dog.ent` 의 남은
컴파일 에러 12개가 0개가 되었습니다.
