...
컴파일러가 만든 작품에는 `id` 가 없으므로 원래 작품 id 를 붙여 줍니다.

### 공유 · 실시간 변수는 사이트의 것이다

`boot` 에 `store: null` 을 넘깁니다. 로컬 실행기는 서버가 없어서 브라우저 저장소에
값을 담아 두지만(AI_TESSVM.md 12장), playentry 에서는 그 값이 사이트의 것입니다.
엔트리도 `isCloud` 변수는 그 자리에서만 읽고 쓰고 작품과 함께 저장될 뿐, 보는 사람이
바꾼 값을 서버로 보내지 않습니다. 여기서 따로 담아 두면 그 사람에게만 보이는 값이
생깁니다.

`isRealTime` 은 읽기도 쓰기도 서버에 물어야 합니다 — 엔트리는 `socket.io` 로
`<cvServer.url>` 의 `/cv` 에 붙어 `action` 으로 `set`·`append`·`insert`·`delete`·
`replace` 를 보냅니다. **그 서버 주소와 접속 토큰(`{url, query, type}`)은 entryjs 가
아니라 playentry 쪽이 `setServerInfo` 로 넘겨 주는 값**이라 여기서는 알 수 없습니다.
...
- **키 입력은 실행기 안에서만** 받습니다(`BootOptions.keyTarget`). 실행 페이지와 달리
  ...
  같이 움직입니다. 그렇게 받은 키는 페이지로 내려보내지 않습니다 — 아래를 봅니다.
- **조작줄을 마우스로 누르면 포커스를 작품에 돌려줍니다.** 단추와 스위치는 눌린 뒤에도
  포커스를 쥐고 있어서, 부스트 스위치를 켠 다음 누른 방향키가 작품이 아니라 페이지로
  갔습니다. `click` 의 `detail` 이 0 이면 키보드로 그 칸을 쓴 사람이므로 그대로 둡니다.
...
| `BootOptions.store`         | 공유·실시간 변수를 둘 곳. 확장은 `null` 을 넘긴다            |
...
`:fullscreen` 과 `.is-full`(자바스크립트가 다는 표시) 둘 다에 같은 규칙을 겁니다.

- **조작줄을 `display: none` 으로 내립니다.** 정지·일시정지·좌표·부스트·전체화면 단추가
  모두 그 안에 있으니 전체화면에서는 아무 메뉴도 남지 않습니다. 나가는 길은 Esc 입니다.
- **알림 줄은 무대 위로 올립니다**(`position: absolute`). 전체화면에서 무대 밑에 줄이
  하나 깔리면 그만큼 작품이 줄어듭니다. 자리를 차지하지 않으니 무대는 화면 전체를 씁니다.
- 단추가 사라지면 **눌렸던 그 단추도 사라져** 키가 아무에게도 가지 않으므로,
  전체화면에 들어가는 순간 실행기(`root`)에 초점을 되돌립니다.

### 우클릭 메뉴 — 조작줄이 닿지 못하는 자리

무대를 우클릭하면 정지·일시정지(또는 시작·이어서 하기)·이전 장면·다음 장면·전체화면이
뜹니다. 엔트리에는 없는 것이고, 여기에 둔 이유는 둘입니다.

- **전체화면에서는 조작줄이 없습니다**(위 참고). 메뉴는 실행기(`root`) 에 붙으므로 그대로
  남고, Esc 말고도 나갈 길이 생깁니다.
- **장면을 넘기는 길이 없었습니다.** 실행 페이지에는 디버그 패널이 있지만 확장에는
  없어서, 그중 손이 자주 가는 것만 여기로 옮겼습니다.

줄은 **열 때 만듭니다.** 그래야 지금 상태 그대로 — 멈춰 있으면 `시작하기`, 첫 장면이면
`이전 장면`이 꺼진 채로 — 나옵니다. 장면 이동은 `vm.ops.startNeighborScene` 이고,
누른 뒤에는 실행기가 다시 키를 가져갑니다.

모양은 **고정 픽셀**입니다. 무대 폭을 따라가는 것은 엔트리가 캔버스 안에 그리는 것
(대답 입력칸·차트 창)뿐이고, 이것과 허용 창은 실행기가 보는 사람에게 말하는 자리입니다.
확장이 얹히는 playentry.org 에는 `input, select, textarea, button { appearance: none;
border-radius: 0; background: none }` 같은 전역 초기화가 있어서, 우리 규칙은 모두
클래스를 하나 앞에 붙여 그보다 무겁게 씁니다(`line-height` 도 0 으로 눌립니다).

### 모르는 블록 알림은 꺼 두는 것이 기본이다

옮기지 못한 블록이 있으면 무대 아래에 한 줄로 알렸는데, 보는 사람에게는 손댈 수 없는
정보라 기본적으로 띄우지 않습니다. 팝업의 **모르는 블록 알리기** 스위치(`notice` 키,
기본 꺼짐)를 켜면 그때부터 보입니다.

문구는 켜져 있든 아니든 실행기가 들고 있으므로, 스위치를 켜면 작품을 다시 불러오지 않고
바로 나타납니다 — `mask` 와 같은 길(`postMessage` → `setShowNotice`)입니다. 실패(빨간 줄)는
이 스위치와 상관없이 언제나 보입니다.

### 설정 창은 단추 하나와 그 아래 설정이다

툴바 단추 뒤의 창(`src/popup`)에서 **실행기 자체는 가운데 둥근 단추 하나**입니다 — 누르면
켜지고 다시 누르면 꺼지며(`role="switch"` · `aria-checked`), 그 상태가 머리줄의 점·칩과
아래 문구에 함께 나타납니다. 벡터·아이디 가리기·모르는 블록 알림은 그 실행기의 설정이라
카드 하나에 묶여 있고, 실행기가 꺼져 있으면 흐려집니다(누를 수는 있습니다).

값은 예전과 같은 키(`enabled`·`svg`·`mask`·`notice`)로 곧바로 저장소에 씁니다. 창을 여는
것만으로는 아무것도 쓰지 않습니다 — 읽어서 그대로 보여 주기만 합니다.
