...
| 문자열 | `"안녕"`, `"줄바꿈\n"`, `"유니코드A"`                        |
| 불리언 | `true`, `false`                                              |
| 색상   | `#ff0000` · `#abc` · `#검정` (16진수 또는 색 이름, `#rrggbb` 로 정규화) |
...
| `break`                                                        | 반복문 하나 빠져나가기                                                               |
| `continue`                                                     | 반복문의 다음 차례로 (다음 프레임에)                                                 |
| `skip`                                                         | 반복문의 다음 차례로 (프레임을 넘기지 않고 바로)                                     |
...
`break`/`continue`/`skip`은 반복문 안에서만 쓸 수 있습니다.
...
| `length(s)`                     | 길이                                   |
| `s[i]`                          | i번째 글자 (1부터)                     |
| `slice(s, from, to)`            | 부분 문자열 (from·to 번째 글자, 양끝 포함) |
| `count(s, t)`                   | t가 나오는 횟수                        |
| `join(a, b)`                    | 이어 붙이기                            |
| `index_of(s, t)`                | t가 처음 나오는 위치(1부터, 없으면 0)  |
...
in 순위 insert 100 at 2        # 2번(1부터) 자리에 끼워 넣기
remove 순위[1]                 # 1번 항목 제거
순위[1] = 95                    # 항목 값 바꾸기
var 개수 = length(순위)
var 있음 = contains(순위, 100)
```

리스트·문자열 인덱스는 항상 **1부터**입니다(엔트리와 같습니다 — 컴파일러가 따로
보정하지 않고 그대로 옮깁니다).
...
전체 키워드는 [tess.ohm](./packages/parser/legacy/tess.ohm)의 `keyword` 규칙(예약어)과 그 아래 나열된
