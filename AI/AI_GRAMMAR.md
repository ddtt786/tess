...
> 지금 돌아가는 문법은 `packages/parser/src/parser/`이고, 구현 노트는 `AI_PARSER.md`에 있다.
> 아래에서 말하는 `src/tess.ohm`은 `packages/parser/legacy/tess.ohm`에 참고용으로 남아 있다.
...
- Tess의 주석은 `#`로 시작하는데, **색상 리터럴도 `#ff0000`처럼 `#`로
  ...
  본문이 **아닐 때만** 주석"이라고 부정 lookahead로 구분한다.
- **색상 본문**은 `#` 바로 뒤의 식별자 글자 묶음(`[\p{L}0-9_]+`) 전체가
  16진수이거나 알려진 색 이름일 때를 말한다(`@tess/core` 의 `colorLiteralLength`).
  글자 묶음 전체가 조건을 만족해야 하므로 `#effect 처리` 는 `t` 에서 걸려 주석으로
  남고, `#검정색으로` 도 이름표에 없어 주석이 된다. 렉서(`tokens.ts` 의
  `ColorLiteral`)와 컴파일러의 주석 수집기(`comments.ts`)가 같은 함수를 쓴다.
#### 색상 리터럴의 정규화

엔트리는 색을 `#RRGGBB` 하나로만 담으므로, AST(`Color` 노드)에 실리는 값은 언제나
그 형태다. `@tess/core` 의 `normalizeColor` 가 다음 규칙으로 옮긴다.

| 적은 것        | 되는 값     | 규칙                                   |
| -------------- | ----------- | -------------------------------------- |
| `#abc`         | `#aabbcc`   | CSS 세 자리 줄임꼴 — 각 자리를 두 번   |
| `#abcd`        | `#aabbcc`   | 네 자리 줄임꼴, 알파는 버린다          |
| `#ff0000`      | `#ff0000`   | 그대로                                 |
| `#ff0000cc`    | `#ff0000`   | 여덟 자리, 알파는 버린다               |
| `#efeff`       | `#efeff0`   | 여섯 자리에 모자라면 `0` 으로 채운다   |
| `#ffffff100`   | `#ffffff`   | 여섯 자리를 넘으면 앞 여섯 자리만      |
| `#검정`·`#red` | `#000000`·`#ff0000` | `NAMED_COLORS` 의 한글·CSS 이름 |

자리 수가 어중간하거나 이름으로 적은 색도 **에러가 아니다**. 이름표에 없는 낱말만
주석으로 남는다. VS Code 문법 강조(`editors/vscode/build-grammar.ts`)도 같은
`NAMED_COLORS` 를 읽어 정규식을 만든다.
...
| 색상 본문   | 글자 묶음 전체가 16진수이거나 색 이름           | `#effect` 처럼 앞부분만 16진수인 주석을 색으로 오인          |
