...
- **저장(`store`) 변수·리스트**: `StorageScope` 에 `store`. 전역 변수·리스트의 "저장 방식"에서 `저장` 을 고르면
  `store var|list …` 로 쓰이고 컴파일하면 `@이름`(Entry Save Manager 규약)이 된다. 목록 태그 `저장`(`SCOPE_LABELS`).
  - `.ent` 불러오기: 오브젝트 소속이 아니고 공유·실시간이 아닌 `@이름` 전역(`@확장프로그램` 제외)은 이름에서 `@` 를 떼고
    `store` 로(`storeName`). `can_save` 는 지금도 `@확장프로그램` 변수 비교로 돌아온다.
  - 미리보기 변수 창 이름은 실행 화면처럼 `@이름`. 값 말풍선의 `fillLists` 는 `store` 리스트를 `@이름` 으로 짝짓는다.
- **어니언 자리**: 모양의 저장 크기(`width/height`)가 아니라 불러올 때와 같은 식으로 놓는다(`onionPlace`, 주소별 캐시) —
  SVG 는 view box 와 `centred` 의 `Math.round(… - minX)`, 그림은 제 픽셀 크기(시트보다 크면 줄여) 가운데. 소수 view box 도
  불러온 그림과 0.02px 안으로 겹친다(예전엔 조금 왼쪽).
- **저장 방식 태그**: 공유 파랑, 실시간 주황, 저장 초록 알약(`.scope-tag.scope-*`).
- **그림판 설정 기억**: 채우기·선 색, 선 굵기, 붓 크기, 글꼴·글자 크기, 선 모양을 `stylechange` 마다 localStorage
  `tessblock.paint` 에 쓰고, 그림판을 열 때와 벡터/비트맵 전환 뒤(새 시트는 기본값) 되돌린다(`restoreSettings`, 되돌리는
  동안은 쓰지 않음). 고른 것이 없을 때만 되돌리므로 도형은 바뀌지 않는다.
- **색 바꾸기**: 채우기와 선이 함께 보일 때(그라데이션 아닐 때) 그 사이 `색 바꾸기` — 두 색을 맞바꾼다(고른 도형에도).
- **가운데 버튼 이동**: 그림판에서 가운데 버튼으로 끌면 도구와 상관없이 보기가 움직인다(`bindMiddlePan`, 호스트에 캡처로
  걸어 모드가 바뀌어도 유지, 매번 지금의 `.pt-viewport` 를 스크롤). painter 의 포인터 처리는 원래 가운데 버튼을 무시한다.
  브라우저의 자동 스크롤(mousedown)·리눅스 붙여넣기(auxclick)는 막는다. 끄는 동안 `.panning` 커서.

## 28. 그림판 채우기(정밀·속도·그룹), 도형 선택, 선택 상자, 색 편집

- **채우기 속도**(painter `enclosedRegion`): 예전엔 마우스를 움직일 때마다 1920×1080 벽 마스크를 flood 하고, 새면 틈 반지름
  4개마다 전체를 다시 flood 했다(호버 1회 ~14ms, 열린 곳은 더). 이제
  - 벽의 열린 영역을 한 번에 이름 붙이고(`labelRegions`, 4-연결, `floodRegion` 과 같은 결과) 호버는 표 읽기.
  - 틈 메우기는 반 크기 벽(`halfWall`, 2×2 중 하나라도 벽이면 벽 — 선이 빠지지 않음)에서, 반지름별 이름표도 캐시.
  - 찾은 영역의 외곽은 영역별로 캐시(`regionResults`, 그림이 바뀌면 `invalidateBarrierCache` 로 모두 비움).
  - `FillTool` 호버는 애니메이션 프레임당 한 번(`hoverFrame`).
  - `floodBorders` 는 타입 배열 큐.
  - 결과: 반복 호버 ~110ms/8회 → 0–1ms. 그림이 바뀐 뒤 첫 영역 36–184ms.
- **채우기 정밀도**(`traceRegion`): 시트 래스터에서 찾은 영역만 그 상자에서 최대 4배(`FILL_DETAIL`, 600만 화소 안)로 다시
  래스터해 찾고(시트에서 찾은 영역 +2px 안으로 가둠 — 메운 틈은 그대로 막힘), 그 안에서
  - 선 밑으로 2.5px(`FILL_UNDER_LINES`)까지 벽 화소로만 자라고(`growIntoWall`), 바깥 빈 곳에 닿는 가장자리 벽 화소는
    되돌려(`keepOffRim`, 선의 흐린 가장자리 뒤로 계단이 비치지 않게), 40px² 이하 구멍은 메운다(`fillPockets`).
  - 윤곽은 `traceMask(ε 0.9 고해상도 px)` + 직선 구간(`ringToPathData`). 예전의 중점 2차 곡선은 꼭짓점을 깎아 "뭉툭"했다.
  - 번짐은 0.3 이하(이미 선 밑).
- **그룹 채우기**: 채우기가 그 선들의 그룹에 들어갈 때 그룹의 변환의 역을 준다(움직인·늘린 그룹에서 자리가 어긋났다).
  그룹 안 예전 채우기 위, 선 아래에 놓는다(예전엔 맨 아래라 다시 채워도 안 보였다). 같은 윤곽의 채우기는 새로 쌓지 않고
  색만 바꾼다(`placeFill`, 그룹 밖도 같음).
- **도형 도구**: 방금 그린 도형은 선택된 채로 있고, 도형 도구 그대로 그 도형이나 손잡이를 누르면 선택 도구처럼 옮기기·
  크기·회전(`ShapeTool` 이 `SelectTool` 에 넘김). 다른 곳을 누르면 새로 그린다. Shift 는 늘 새로 그린다.
- **선택 상자**(`itemsInMarquee`): 상자 안에 다 든 것, 또는 윤곽(경로를 따라 표본)이 상자를 지나는 것. 상자를 감싸기만 하는
  도형은 고르지 않는다. Alt 는 예전처럼 다 든 것만.
- **색 편집**(`ColourField`): 색 칩(체커 위 색, 코드·불투명도 %, 없음은 빨간 사선)을 누르면 아래에 편집기 — 색상·채도·밝기·
  불투명도 막대(막대 자체가 바뀌는 범위를 그라데이션으로 보인다), 색 코드, 스포이드, 없음. 색은 `#rrggbb` 또는 `#rrggbbaa`.
  그라데이션 시작·끝도 같은 칩이고 거기서 "없음"은 완전 투명. 편집 중 색상값은 따로 들고 있어 회색·검정을 지나도 색상이
  남는다. 밖 누르기·Esc 로 닫힌다.
- 선택 도구로 도형을 고르면 칩은 그 도형의 색(`selectionStyle`, 첫 칠해진 부분)을 보인다. 색 바꾸기도 그 색끼리.

## 29. 스포이드 새는 누르기, 폰트 넣기, Shift 스냅, 폴더 표시, 참/거짓 변수·함수, 돌려주기 자리

- **스포이드**: 대체 방식(`pickFromSheet`)은 시트 래스터를 기다리지 않고 바로 듣는다(래스터는 함께 진행, 누른 점은 끝난 뒤
  읽음). 고르는 동안과 끝난 뒤 한 프레임은 `holdPresses` 가 페이지의 누르기·떼기·클릭을 삼킨다(브라우저 `EyeDropper` 도).
  `Dropper` 버튼은 `setTimeout(0)` 없이 바로 시작 — 예전엔 그 틈의 빠른 클릭이 채우기 도구로 가서 채워졌다.
- **폰트 넣기**(`model/font-embed.ts` `embedFonts`): 이미지로 그려지는 SVG(실행기·엔트리·썸네일·비트맵 변환)는 페이지 폰트를
  못 써 기본 폰트가 된다. 벡터 모양을 저장할 때(`flushPainter`)와 비트맵으로 바꿀 때(painter 옵션 `prepareSVG`) `<text>` 의
  글꼴·굵기별로 쓴 글자만 담은 서브셋 폰트(opentype.js 로 woff 를 읽어 OTF 로 씀)를 `<defs><style data-tess-fonts>` 의
  `@font-face` 로 넣는다(다음 저장 때 바꿔 씀). 글꼴 파일은 `FONT_FILES`(엔트리 CDN woff — 스타일시트는 CORS 로 못 읽고
  글꼴 파일은 읽힌다; 나눔고딕은 CSS 가 woff2 조각뿐이라 `NanumGothic-Regular/Bold.woff`). 목록에 없거나 못 받은 글꼴은
  그대로 둔다. 첫 저장 ~0.4s(받기·읽기, 이후 캐시 ~6ms), 두 글꼴 몇 글자 +8KB. 실제 웹폰트 렌더와 픽셀 차 466(예전 7894).
- **Shift 이동 스냅**(painter `SelectTool`): Shift 로 옮기면 고른 것의 왼·가운데·오른(위·가운데·아래)이 다른 도형과 시트의
  같은 선에 8px 안이면 붙고 안내선(`drawLines`). 예전 Shift 의 가로/세로 고정은 없앴다. Shift 없이는 예전처럼 시트 가운데만.
- **폴더 표시**: 오브젝트 정보 이름 칸 앞 `폴더/`(`.detail-folder`), 편집 탭 오른쪽 위 `폴더/이름`.
- **변수를 참/거짓으로**: `data_variable_true`("%1 이(가) 참인가?", 육각형) — 변수 이름만 쓰고, Tess 컴파일러가 조건 자리에서
  `== TRUE` 로 바꾼다(`not`·`and` 안도 같음).
- **참/거짓을 돌려주는 함수**: 돌려주기 값이 육각형 블록(또는 그런 함수의 호출)이면 값 호출 블록 출력이 `Boolean`
  (`returnsBoolean` — 인라인은 정의 블록에서, 저장된 함수는 상태에서). 바뀌면 놓인 호출 블록을 다시 만든다(`refreshCallBlocks`).
- **돌려주기는 끝에 하나**: 엔트리 함수의 결과는 본문 뒤 식 하나. `func_return` 은 아래 연결이 없고, 정의 블록 본문 자체에 있을
  때만 쓰인다 — `if`·반복 안이면 꺼지고(`tess_return_place`) 경고. 값 함수 여부도 본문 맨 위 단계의 돌려주기로만 본다
  (`topReturn`/`topReturnState`).
- **함수 팔레트**: 돌려주는 함수는 값(참/거짓이면 육각형) 호출 블록 하나만, 아닌 함수는 문장 호출 블록 하나만 보인다(검색도 같음).
  이미 놓인 값 함수의 문장 호출은 그대로 컴파일된다. 팔레트 다시 그리기 키에 `returnsBoolean` 포함.
